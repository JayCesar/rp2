...
try:
    ...
    logger.error("Portuguese spaCy model not found. Please install with: python -m spacy download pt_core_news_sm")
    sys.exit(1)
    ...
    try:
        ...
        for token in doc:
            ...
            if (not token.is_stop and 
                ...
                token.text.strip() not in string.punctuation):
...
class BERTimbauC1Predictor:
    ...
    def __init__(self, model_path: Union[str, pathlib.Path], device: Optional[torch.device] = None):
        self.device = device if device else torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.model_path = pathlib.Path(model_path)

        # Load the saved model
        checkpoint = torch.load(self.model_path / "bertimbau_c1_model.pth", map_location=self.device, weights_only=False)
    ...
    def predict_single_essay(self, essay_text: str, use_preprocessing: bool = True) -> float:
        ...
        with torch.no_grad():
            ...
            prediction = normalized_prediction * (self.c1_max - self.c1_min) + self.c1_min
    ...
    def predict_batch(self, essay_texts: List[str], use_preprocessing: bool = True) -> List[float]:
        ...
        for essay_text in essay_texts:
            prediction = self.predict_single_essay(essay_text, use_preprocessing=use_preprocessing)
            predictions.append(prediction)

        return predictions

    def predict_with_confidence(self, essay_text: str, num_samples: int = 5, use_preprocessing: bool = True) -> Dict[str, Union[float, List[float]]]:
        """Make prediction with confidence estimation using multiple forward passes.
        ...
        for _ in range(num_samples):
            pred = self.predict_single_essay(essay_text, use_preprocessing=use_preprocessing)
...
def demo_predictions():
    ...
    for i, essay in enumerate(example_essays, 1):
        ...
        print(f"Essay {i} - With confidence: {confidence_result['prediction']:.4f} ± {confidence_result['confidence_std']:.4f}")
...
def predict_from_csv():
    ...
    relevant_columns = ["essay", "c1"]
    SAMPLE_SIZE = None # Set to None to load all rows
    ...
    df = (
        ...
        .head(SAMPLE_SIZE if SAMPLE_SIZE is not None else SAMPLE_SIZE_UPPER_BOUND)
        .collect()
    )
    print(f"Loaded {len(df)} rows from CSV")

    print(f"[DEBUG] Printing first {SAMPLE_SIZE if SAMPLE_SIZE else len(df)} rows of the CSV:")
    print(df.head(SAMPLE_SIZE if SAMPLE_SIZE else len(df)))
    ...
    for i, row in enumerate(df.iter_rows(named=True), 1):
        if row["essay"] is not None:
            print(f"Processing essay {i}...")
            prediction = predictor.predict_single_essay(row["essay"])
            predictions.append(prediction)
    ...
    if len(predictions) > 0:
        mse = np.mean([(pred - actual) ** 2 for pred, actual in zip(predictions, actual_c1_values)])
        mae = np.mean([abs(pred - actual) for pred, actual in zip(predictions, actual_c1_values)])
