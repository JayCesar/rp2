"""Conv1D Trainer with CrossEntropy Loss for Classification
...
Mirrors the structure of BLSTM CE trainer but adapted for Conv1D:
...
# Import CE-specific components and mappings
try:
    from .conv1d_cross_entropy_loss import (
        Conv1DClassifier,
...
def calculate_class_weights(class_frequencies: list[int]) -> torch.Tensor:
    """Calculate class weights from frequencies using inverse frequency.
    ...
    for freq in class_frequencies:
        if freq == 0:
            ...
            weight = total / (len(class_frequencies) * freq)
            weights.append(weight)
    return torch.tensor(weights, dtype=torch.float32)


class Conv1DCETrainer:
    """Trainer for Conv1D classification with CrossEntropyLoss."""
    ...
    def __init__(
        ...
        if class_frequencies is not None:
            class_weights = calculate_class_weights(class_frequencies).to(self.device)
        else:
            class_weights = None
    ...
    def _setup_optimizer(self) -> torch.optim.Optimizer:
        if self.train_config.optimizer == "adamw":
            ...
            if self.device.type == "cuda":
                try:
                    return torch.optim.AdamW(self.model.parameters(), fused=True, **kwargs)
                except Exception:
    ...
    def _validate_epoch(self) -> tuple[float, dict[str, float], list[dict[str, str | float | int]]]:
        ...
        with torch.no_grad():
            for batch in self.val_loader:
                ...
                # Validation CE loss on class indices
    ...
    def _save_checkpoint(self, is_best: bool = False, metrics: dict[str, float] | None = None) -> None:
        ckpt = {
