"""Focal Loss Components for Conv1D

This module provides classification components for training Conv1D models
with FocalLoss instead of regression loss.

Reuses BLSTM infrastructure:
...
# Import BLSTM mapping utilities
...
class Conv1DClassifier(nn.Module):
    """1D Convolutional Neural Network for essay C1 score classification with FocalLoss.
    
    Mirrors Conv1DRegressor encoder but replaces regression head with
    6-way classification head for {0, 40, 80, 120, 160, 200}.
    
    Forward returns raw logits [batch_size, 6] for focal loss.
    ...
    def forward(
        ...
        Returns:
            logits: [batch_size, 6] raw logits for FocalLoss
        """
