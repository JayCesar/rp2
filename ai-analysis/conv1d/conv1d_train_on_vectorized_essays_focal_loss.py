#!/usr/bin/env python3
"""Conv1D Training on Vectorized Essays with Focal Loss

Trains a Conv1DClassifier on essay BERT embeddings using FocalLoss
for 6-class C1 score classification {0, 40, 80, 120, 160, 200}.

Mirrors blstm_train_on_vectorized_essays_focal_loss.py but uses Conv1D:
- Conv1DClassifier with 6 logits output
- FocalLoss on class indices
- Predictions converted back to score space for metrics
- Output directory: runs/vectorized_essays_focal_loss/conv1d_model/
...
def create_component1_config_for_vectorized_essays() -> ModelConfig:
    """Create ModelConfig for Component 1 vectorized essays with FocalLoss."""
    return ModelConfig(
...
def _load_vectorized_dataframe(max_samples: Optional[int]) -> pl.DataFrame:
    """Load full vectorized-essays dataset for FocalLoss training/gamma search.

    Mirrors the existing single-run loading logic so gamma-search uses the same
    source and filters. The optional ``max_samples`` cap is applied later to the
    training split only; here we always load the full dataset (subject to an    internal hard cap to avoid overflow).
    """
    parquet_file = (
        project_root
        / "generated_datasets"
    ...
    logger.info(f"[gamma-search] Loaded vectorized dataset with {len(dataset)} essays")
    logger.info(
        f"[gamma-search] Essay token embeddings shape: {dataset.schema['essay_token_embeddings']}"
    )
    return dataset


def _split_vectorized_dataset(
...
def dataloaders_factory_vectorized(max_samples: Optional[int]):
    """Factory for Conv1D vectorized-essays DataLoaders used by run_gamma_search.

    Returns train/val/test loaders built from the same dataset and split
    strategy used in the original single-run training flow.
    """
    dataset, train_dataset, val_dataset, test_dataset = _split_vectorized_dataset(
...
def compute_alpha_for_vectorized(
    max_samples: Optional[int],
    train_dataset: Optional[Any] = None,
) -> torch.Tensor:
    """Compute class-weight alpha vector for FocalLoss from training labels.

    If ``train_dataset`` is provided, it is used directly and no new dataset
    is loaded or split. Otherwise, this function falls back to loading and
    splitting via ``_split_vectorized_dataset`` (backward compatible).
    """
    if train_dataset is None:
        _, train_dataset, _, _ = _split_vectorized_dataset(max_samples=max_samples)

    class_frequencies = get_class_frequencies(train_dataset)
    logger.info(
        "[gamma-search] Training set class distribution (vectorized essays, capped if requested):"
    )
    print_class_distribution(class_frequencies)
    return calculate_alpha_from_frequency(class_frequencies)


def train_on_vectorized_essays_fl(device: torch.device) -> dict[str, float]:
    """Train Component 1 using Conv1DClassifier with FocalLoss on embeddings."""
    logger.info("Training Component 1 with FL on Vectorized Essays (Conv1D)")
    ...
    model_config = create_component1_config_for_vectorized_essays()
    train_config = create_training_config()
    ...
    # Setup training
    output_dir = Path(__file__).parent / "runs" / "vectorized_essays_focal_loss" / "conv1d_model"
    ...
    summary_path = Path(output_dir) / "component1_vectorized_essays_fl_summary.txt"
    with open(summary_path, "w") as f:
        f.write("Component 1 Training on Vectorized Essays with FocalLoss (Conv1D)\n")
        f.write("=" * 50 + "\n\n")
...
def show_and_save_metrics(
    ...
    logger.info("\n" + "=" * 70)
    logger.info("COMPONENT 1 FL TRAINING ON VECTORIZED ESSAYS COMPLETED (CONV1D)")
    logger.info("=" * 70)
...
def main() -> int:
    """Main training workflow for Component 1 FL on vectorized essays (Conv1D).

    This entrypoint now **always** runs a FocalLoss gamma sweep. Single-run
    training without a sweep is no longer supported.
    """
    print(f"\n{'=' * 50}")
    print("Component 1 Conv1D FL Training on Vectorized Essays (gamma sweep)")
    print(f"{'=' * 50}")

    logger.info(
        "Starting Component 1 FL Training on Vectorized Essays (Conv1D, gamma sweep)"
    )
    logger.info("=" * 70)

    parser = argparse.ArgumentParser(
        description=(
            "Conv1D FocalLoss training on vectorized essays using a mandatory gamma sweep"
        ),
    )
    parser.add_argument(
        "--gamma-grid",
    ...
    print("\n[gamma-search] Gamma grid for FocalLoss sweep (vectorized essays):")
    ...
    # Pre-load dataset and splits once using optional cap
    full_dataset, train_dataset, val_dataset, test_dataset = _split_vectorized_dataset(
    ...
    try:
        logger.info("Running FocalLoss gamma sweep on vectorized essays (Conv1D)")

        # Compute alpha weights from (optionally capped) training split without reloading
        alpha = compute_alpha_for_vectorized(
            args.max_samples,
            train_dataset=train_dataset,
        )

        # DataLoaders factory reusing the pre-split datasets
        def dataloaders_factory_for_sweep(max_samples_inner: Optional[int]):
            # ``max_samples_inner`` is ignored because ``args.max_samples`` has already
            # been applied in the initial split above.
            train_config = create_training_config()
            pin_memory = device.type == "cuda"
        ...
        # Trainer factory capturing device, target scaler and epochs-per-gamma from CLI
        def trainer_factory_conv1d_vectorized(
            out_dir, train_loader, val_loader, test_loader
        ...
        print("\nGamma sweep results (vectorized essays, Conv1D, FocalLoss):")
