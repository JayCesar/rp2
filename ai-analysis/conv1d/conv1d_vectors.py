"""Train Conv1D model on BERTimbau token embeddings for C1 score prediction

Architecture based on Table 4 (Comp. 1) from CLaRiCe paper.
Optimized for performance with AMP, pin_memory, and non_blocking transfers.
...
def main():
    """Main training workflow for Conv1D on vectorized essays."""
    logger.info("=" * 70)
    logger.info("Conv1D Training on Vectorized Essays (BERTimbau Embeddings)")
    logger.info("=" * 70)
    ...
    data_path = project_root / "generated_datasets" / "extended_essay-br_preprocessed_for_BLSTM.parquet"
    ...
    df = (
        ...
        .select(["c1", "essay_token_embeddings"])
        .drop_nulls()
        .unique()
        .collect()
    )
    ...
    # Model configuration (Table 4, Comp. 1)
    ...
    output_dir = script_path.parent / "runs" / "vectorized_essays"
    ...
    summary = []
    summary.append("Conv1D Training on Vectorized Essays\n" + "=" * 70 + "\n")
