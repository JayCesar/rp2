...
def test_preprocessing_pipeline():
    ...
    with open(
        "generated_datasets/extended_essay-br_preprocessed_for_BERT.json", "r"
    ) as f:
        data = json.load(f)
...
def test_bilstm_training():
    ...
    try:
        ...
        if len(dataset) < 4:
            logger.warning(
                "Dataset too small for proper split, using all data for both train and val"
            )
...
def main():
    ...
    if success:
        logger.info(
            "🎉 ALL TESTS PASSED! Complete BiLSTM pipeline is working correctly."
        )
