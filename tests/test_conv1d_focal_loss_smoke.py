#!/usr/bin/env python3
"""Smoke tests for Conv1D Focal Loss training integration.
...
Mirrors test_conv1d_cross_entropy_loss_smoke.py but for Focal Loss.
"""
...
from conv1d_trainer_focal_loss import Conv1DFLTrainer
...
def smoke_output_dir(tmp_path):
    ...
    output_dir = tmp_path / "smoke_test_conv1d_fl"
...
def test_conv1d_fl_training_smoke(tiny_dataset, smoke_configs, smoke_output_dir):
    """Smoke test: Run 1 epoch of Conv1D FL training on tiny dataset."""
    set_seed(42)
    ...
    # Verify all targets are valid FL scores
    ...
    assert checkpoint["loss_type"] == "FocalLoss", "Wrong loss_type in checkpoint"
...
def test_conv1d_fl_features_script_imports():
    """Test that Conv1D FL training scripts can be imported without errors."""
    try:
        from conv1d_train_on_features_focal_loss import (
            create_component1_config_for_features,
            create_training_config,
        )
        from conv1d_train_on_vectorized_essays_focal_loss import (
        ...
        pytest.fail(f"Failed to import Conv1D FL training scripts: {e}")
